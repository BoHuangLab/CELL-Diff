...
def prepare_img_hpa(dataroot: Path):
    nucleus_img = to_tensor(Image.open(os.path.join(dataroot / "nucleus.png")).convert("L"))
    ER_img = to_tensor(Image.open(os.path.join(dataroot / "ER.png")).convert("L"))
    microtubule_img = to_tensor(Image.open(os.path.join(dataroot / "microtubule.png")).convert("L"))

    cell_img = torch.cat([nucleus_img, ER_img, microtubule_img], dim=0).unsqueeze(0)
...
def main(args) -> None:
    ...
    cell_img = prepare_img_hpa(Path(args.cell_morphology_image_path)).to(device)
    ...
    cat_img = torch.cat([torch.full_like(pred_protein_img, -1), pred_protein_img, cell_img[:, :1]], dim=1)
