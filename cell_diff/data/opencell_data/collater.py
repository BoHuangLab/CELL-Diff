...
from cell_diff.data.hpa_data.vocabulary import Alphabet
...
def collate_fn(samples: List[dict], 
    ...
    samples = [
        s
        for s in samples
        if s["protein_seq"].size(0) <= max_protein_sequence_len and s["protein_seq"].size(0) > min_protein_sequence_len
    ]
    ...
    batch["prot_id"] = [s['prot_id'] for s in samples]
